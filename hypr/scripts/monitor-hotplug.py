...
BUILTIN_MONITOR_DISABLED_CONFIG = f"{BUILTIN_MONITOR},disable"
...
def get_monitor_state() -> tuple[bool, list[str]]:
    ...
    Parses ``hyprctl monitors -j`` JSON output.  Each monitor object has a
    ``name`` and ``disabled`` field.

    Returns
    -------
    builtin_active:
        True when eDP-1 is present and not disabled.
    external_names:
...
def enable_builtin() -> None:
    """Ask Hyprland to enable the built-in display with the standard config."""
...
def disable_builtin() -> None:
    """Ask Hyprland to disable the built-in display (dock mode)."""
    log.info("Disabling %s (dock mode)", BUILTIN_MONITOR)
    subprocess.run(
        ["hyprctl", "keyword", "monitor", BUILTIN_MONITOR_DISABLED_CONFIG],
        check=True,
    )
...
def settle_and_act() -> None:
    """Examine the stable monitor topology and switch modes as needed.

    Called by the debounce timer once the topology has been quiet for
    SETTLE_DELAY seconds.  Four-state decision table:
    ...
    try:
        ...
        log.info(
            "Settled state: builtin_active=%s, externals=%s",
            builtin_active,
            external_names,
        )
